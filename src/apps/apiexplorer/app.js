	...
	var app = {
		...
		bindEvents: function(parent, callback) {
			...
			container.find('.content').on('click', '.item__head', function() {
				...
				if ($(item_body).hasClass('dnone') && $(item_response).text().length === 0) {
					...
					if (arr_params !== null) {
						arr_params = $.map(arr_params, function(elem) {
									...
									self.propsToDOM(data.data.properties, 0, '', function() {
										...
										$(item_data).on('click', '.add-input', function() {
											...
											$(item_data).find('.block-costum')
												.append('<div class="input-block input-block-prop">'
													+ '<input id="' + id + '" data-level="0" type="text" '
													+ 'placeholder="Costum key" class="item__input item__input-prop" />'
													+ '</div>');

											self.propComplete($(item_data)
												.find('.input-block-prop:last-child'), '#' + id, '', 0, function() {
												$(item_body)
													.css('max-height', $(item_response).height()
														+ $(item_request).height() + 120 + 'px');
											});

											$(item_body).css('max-height', $(item_response).height()
												+ $(item_request).height() + 120 + 'px');
										});
									});
								},
			...
			$('body').on('keyup', '#search', function(e) {
				...
				if ($(this).val() !== '') {
					timeoutID = setTimeout(function() {
						...
						parent.find('.block').toggle(false);
						$.each(parent.find('.block'), function() {
		...
		propComplete: function(parent, selector, parentProp, level, callback) {
			...
			$(parent).find(selector).autocomplete({
				...
				select: function(event, ui) {
					...
					$(parent).find('.item__input').filter(function() {
						return $(this).attr('data-level') > $(thisElem).attr('data-level');
					}).remove();
					level = parseInt($(thisElem).attr('data-level'), 10) + 1;

					if (ui.item.type === 'select') {
						$(parent).append('<input id="' + id + '" data-level="'
							+ level + '" type="text" placeholder="Costum key" class="item__input item__input-prop" />');
						$(parent).find('#' + id).attr('style', 'margin-left: ' + level + 'rem !important');
						self.propComplete(parent, '#' + id, ui.item.value, level, function() {
							callback && callback();
						});
					} else {
						$(parent).append('<input id="' + id + '" data-level="' + level + '" type="' + ui.item.type
							+ '" placeholder="Costum value" class="item__input item__input-value" />');
						$(parent).find('#' + id).attr('style', 'margin-left: ' + level + 'rem !important');
						callback && callback();
					}
				}
			...
			$.each(props, function(i, item) {
				if (typeof item.type !== 'undefined' && item.type === 'object'
					...
					if (typeof item.type !== 'undefined' && item.type === 'object'
						...
						if (item.type === 'number' || item.type === 'string' || item.type === 'integer'
							|| item.type === 'boolean') {
							domProps.push({
								value: i,
								label: item.name || i.replace('_', ' '),
		...
		getValidData: function(item_data, item_title, callback) {
			...
			$(item_data).find('.input-block:not(.input-block-prop) .item__input').each(function(i, elem) {
				if ($(elem).val().length !== 0) {
					params_data[$(elem).attr('name')] = ($(elem).attr('type') === 'checkbox')
						? $(elem).is(':checked')
						: $(elem).val();
				}
			});
			...
			$(item_data).find('.input-block-prop').each(function(i, elem) {
				...
				if ($(costumValue).val().length !== 0 || $(costumValue).attr('type') === 'checkbox') {
					costumParams[$(elem).find('.item__input-prop').last().val()] = ($(costumValue).attr('type') === 'checkbox')
						? $(costumValue).is(':checked')
						: $(costumValue).val();
			...
			var url_request = monster.config.api.default + $(item_title).text();
			...
			$.ajax({
				...
				success: function(data) {
					...
					if (data.next_start_key) {
						$('<a class="js-next-request btn"><i class="fa fa-paper-plane"></i> '
							+ self.i18n.active().apiexplorer.nextRequestBtnText + '</a>')
							.appendTo($buttonsBox)
							.on('click', function(e) {
								e.preventDefault();
					...
					item_response.html('<br/>'
						+ '<div class="pre_block">'
						+ '<pre class="pre">URL: <span class="copy-text">" ' + url + '</span>'
						+ '</pre>'
						+ '<div class="copy-btn fa fa-copy"></div>'
						+ '</div><br/>'
						+ '<div class="pre_block"><pre class="pre"><span class="copy-text">'
						+ JSON.stringify(data, null, '   ')
						+ '</span></pre><div class="copy-btn fa fa-copy"></div></div>');
				...
				error: function(data) {
					...
					item_response.html('<br/><div class="pre_block"><pre class="pre">URL: <span class="copy-text">'
						+ url + '</span></pre><div class="copy-btn fa fa-copy"></div></div><br/><div class="pre_block">'
						+ '<pre class="pre"><span class="copy-text">' + JSON.stringify(data, null, '   ')
						+ '</span></pre><div class="copy-btn fa fa-copy"></div></div>');
					item_body.css('max-height', item_response.height() + item_request.height() + 50 + 'px');
					item_response.find('.pre').each(function(i, block) {
						hljs.highlightBlock(block);
		...
		getLeftTitles: function() {
			var leftTitles = '';

			$.each(methodsGenerator, function(i, elem) {
				leftTitles += ''
					+ '<div class="left__title" data-title="' + elem.title.toLowerCase() + '">'
					+ elem.title
					+ '</div>';
			});
			...
			self.callApi({
				...
				success: function(data) {
					...
					if (resource_ids.length > 0) {
						...
						var $el = $('.' + name_res + 'Id');
						$el.autocomplete({ source: resource_options, minLength: 0, delay: 0 });
		...
		getPropName: function(elem, prop) {
			return this.defElem(elem.name) || this.defElem(elem.numbers)[0]
				|| this.defElem(this.defElem(elem.featurecode).name)
				|| this.defElem(elem[prop])
		...
		getKazooMethods: function() {
			$.ajax({
				url: 'apps/' + this.name + '/lib/kazoo.methods.json',
				async: false,
				crossDomain: true,
				dataType: 'text',
				success: function(data) {
		...
		getAnimationFlag: function() {
			...
			if ((localStorage.getItem('animation') !== null
				...
				|| (localStorage.getItem('animation') !== 'true'
					&& monster.config.developerFlags.animation === false)) {
				animation = false;
			}
		},
